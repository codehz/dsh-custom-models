...
export function SettingsSection({ api, t }) {
    ...
    return _jsxs("section", { className: "cm-root", children: [_jsxs("header", { className: "cm-head", children: [_jsxs("div", { children: [_jsx("h2", { children: t("title") }), _jsx("p", { className: "cm-sub", children: t("subtitle") })] }), _jsx(Button, { type: "button", variant: "primary", disabled: !snapshot.writable, onClick: () => {
                ...
                : null, snapshot.error !== "" ? _jsxs("div", { className: "cm-status cm-error", role: "alert", children: [snapshot.error, " ", _jsx(Button, { type: "button", size: "sm", onClick: () => void reload(api), children: t("retry") })] }) : null, snapshot.loading && snapshot.namespace === undefined ? _jsx("p", { children: t("loading") }) : _jsxs("div", { className: "cm-layout", children: [_jsxs("nav", { className: "cm-list", "aria-label": t("title"), children: [routes.length === 0 ? _jsx("p", { className: "cm-empty", children: t("empty") }) : null, routes.map((route) => _jsxs("button", { type: "button", className: "cm-provider", "aria-current": selected === route, "aria-controls": "cm-provider-editor", onClick: () => choose(route), children: [_jsx("span", { className: "cm-provider-title", children: nameOf(layers.resolved[route], route) }), _jsxs("span", { className: "cm-badge", children: [route, " \u00B7 ", snapshot.active.has(route) ? t("active") : t("inactive"), " \u00B7 ", Object.hasOwn(layers.user, route) ? t("user") : t("base")] })] }, route))] }), draft !== undefined ? _jsxs("fieldset", { id: "cm-provider-editor", className: "cm-editor", disabled: disabled, children: [_jsx("legend", { className: "cm-sr-only", children: draft.displayName || draft.route || t("add") }), _jsxs("div", { className: "cm-editor-head", children: [_jsxs("div", { className: "cm-editor-identity", children: [_jsx("strong", { className: "cm-editor-title", children: draft.displayName || draft.route || t("add") }), draft.route !== "" ? _jsx("span", { className: "cm-editor-route", children: draft.route }) : null] }), selected !== undefined ? _jsxs("span", { className: "cm-badge", children: [snapshot.active.has(selected) ? t("active") : t("inactive"), " \u00B7 ", selectedHasUserOverride ? t("user") : t("base")] }) : null] }), _jsxs("div", { className: "cm-grid", children: [_jsx(Field, { label: t("route"), error: routeCollision ? t("routeExists") : validationMessage(validation?.errors.route), children: _jsx(Input, { value: draft.route, disabled: selected !== undefined || disabled, onChange: (event) => edit((value) => { value.route = event.target.value; }) }) }), _jsx(Field, { label: t("displayName"), children: _jsx(Input, { value: draft.displayName, onChange: (event) => edit((value) => { value.displayName = event.target.value; }) }) }), _jsx(Field, { label: t("api"), children: _jsxs("select", { value: draft.api, onChange: (event) => edit((value) => {

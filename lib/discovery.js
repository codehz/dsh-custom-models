import { LlmError, attributionHeaders, normalizeApiKey, } from "@deepseek-ai/dsh-llm";
...
function usableProbeKey(raw) {
    ...
    throw new LlmError(checked.reason === "empty"
        ...
        : "this provider's API key contains characters no HTTP header can carry; paste the raw key only", "INVALID_CREDENTIAL");
}
/**
 * Interrogate one draft provider endpoint via the OpenAI-compatible
 * `GET {baseURL}/models` listing.
 */
export async function discoverModels(request, storedApiKey, extraHeaders) {
    if (request.baseURL === undefined || request.baseURL.length === 0) {
        throw new LlmError("set a baseURL, or enter this provider's models by hand", "DISCOVERY_FAILED");
    }
    const api = request.api ?? "openai-completions";
    if (!LISTABLE_PROTOCOLS.has(api)) {
        throw new LlmError('protocol "' + api + '" has no model listing this build can read; enter this provider\'s models by hand', "DISCOVERY_UNSUPPORTED");
    }
    ...
    if (!response.ok) {
        throw new LlmError(url + " answered " + response.status +
            (response.status === 401 || response.status === 403 ? "; check the API key" : ""), "DISCOVERY_FAILED");
    }
    let text;
    try {

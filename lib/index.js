...
const reasoningEffortsSchema = z.dict(z.union([z.string(), z.const(null)]), z.union(THINKING_LEVELS));
...
function declaredCompat(compat) {
    if (compat === undefined)
        return undefined;
        ...
        const models = profile.models.map((entry, index) => {
            ...
            if (defaultEffort !== undefined) {
                if (entry.reasoningEfforts === undefined ||
                    ...
                    throw new Error("dsh-custom-models: provider route '" + provider + "' model '" + entry.id +
...
export function apply(ctx, config = {}) {
    ...
    const reconcile = () => {
        ...
        try {
            ...
            if (!isRecord(error) || error.code !== "INVARIANT")
                normalized = previous;
            throw error;
        }
    };
    ...
    ctx.llm.registerModelDiscovery(SETTINGS_NS, (request) => {
        ...
        return discoverModels(request, () => storedApiKey(request.provider), profile?.headers);
    });
    ...
    const reconcileSettings = () => {
        try {
            ...
            ctx.logger.error("dsh-custom-models: keeping the previous provider registration after a refused settings update");
